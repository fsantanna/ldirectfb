...
static int l_new (lua_State* L)
{
	...
	luaL_getmetatable(L, "directfb.IDirectFBDisplayLayer");
...
static int l_gc (lua_State* L)
{
	...
	IDirectFBDisplayLayer* layer = * (IDirectFBDisplayLayer**) luaL_checkudata(L, 1, "directfb.IDirectFBDisplayLayer");
	layer->Release(layer);
	return 0;
}
...
{
	// [ ... ]
	CreateInstance(L, "directfb.IDirectFBWindow");
	// [ instance ]
}
...
LUALIB_API int luaopen_directfb_IDirectFBDisplayLayer (lua_State* L)
{
	...
	lua_pushstring(L, "directfb.IDirectFBSurface"); // [ ... | require | "IDFBSurface" ]
	lua_call(L, 1, 1);                              // [ ... | _M ]
	lua_pop(L,1);                                   // [ ... ]
